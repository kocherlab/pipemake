rule all:
    input:
        f"Annotations_dir/Trinotate/{config['species']}_OGS_{config['assembly_version']}.{config['annotation_version']}.report.tsv",


rule annotations_orthofinder:
    input:
        directory("Homology/OthroFinder/Input"),
    output:
        "Homology/OthroFinder/Output/Orthogroups/Orthogroups.tsv",
        "Homology/OthroFinder/Output/WorkingDirectory.tar.gz",
    params:
        out_dir="Homology/Output",
        out_suffix="tmp",
    singularity:
