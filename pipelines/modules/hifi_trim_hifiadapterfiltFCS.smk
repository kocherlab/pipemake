...
rule pacbio_bam_to_fasta:
    ...
    threads: 1
    shell:
        "bamtools convert -format fasta -in {input} -out {output} &> {log}"
...
rule hifi_fastq_to_fasta:
    ...
    threads: 1
    shell:
        "seqkit fq2fa {input} -o {output} &> {log}"
...
rule hifi_reads_screen_fcs_adaptor:
    ...
    log:
        "logs/fcs-adaptor/{sample}.log",
    params:
        out_prefix=subpath(output[0], parent=True),
    ...
    threads: 1
    shell:
        "av_screen_x {input} --output {params.out_prefix} {params.prok} {params.euk} &> {log}"
...
rule hifi_screen_hifiadapterfiltFCS:
    ...
    log:
        "logs/hifiadapterfiltFCS/{sample}.log",
    params:
    ...
    threads: 4
    shell:
        "hifiadapterfiltFCS.sh -r {input.reads} -f {input.fcs_adaptor_report} -o {params.out_dir} -t {threads} &> {log}"
