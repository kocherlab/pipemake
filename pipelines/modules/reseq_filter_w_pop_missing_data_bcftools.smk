...
rule pop_vcf_bcftools:
	input:
		vcf_file=os.path.join(config['paths']['reseq_filtered_vcf_dir'], f"{config['species']}_{config['assembly_version']}.filtered.woMD.vcf.gz"),
		pop_file=os.path.join(config['paths']['models_dir'], 'BCFtools', "{model_pop}.pop")
	output:
		vcf_file=temp(os.path.join(config['paths']['reseq_filtered_vcf_dir'], f"{config['species']}_{config['assembly_version']}.filtered.woMD.{{model_pop}}.vcf.gz")),
		idx_file=temp(os.path.join(config['paths']['reseq_filtered_vcf_dir'], f"{config['species']}_{config['assembly_version']}.filtered.woMD.{{model_pop}}.vcf.gz.csi"))
	params:
...
def aggregate_pop_reseq (wildcards):
	...
	return {'vcf_file': expand(os.path.join(config['paths']['reseq_filtered_vcf_dir'], f"{config['species']}_{config['assembly_version']}.filtered.woMD.{{model_pop}}.vcf.gz"), model_pop = glob_wildcards(os.path.join(checkpoint_output, "{model_pop}.pop")).model_pop),
			'idx_file': expand(os.path.join(config['paths']['reseq_filtered_vcf_dir'], f"{config['species']}_{config['assembly_version']}.filtered.woMD.{{model_pop}}.vcf.gz.csi"), model_pop = glob_wildcards(os.path.join(checkpoint_output, "{model_pop}.pop")).model_pop)}

rule isec_pop_vcfs_bcftools:
	input:
		unpack(aggregate_pop_reseq)
	output:
		temp(os.path.join(config['paths']['reseq_filtered_vcf_dir'], f"{config['species']}_{config['assembly_version']}.filtered.missing_data.sites"))
	params:
...
rule filter_pops_missing_data_vcf_bcftools:
	input:
		vcf_file=os.path.join(config['paths']['reseq_filtered_vcf_dir'], f"{config['species']}_{config['assembly_version']}.filtered.woMD.vcf.gz"),
		idx_file=os.path.join(config['paths']['reseq_filtered_vcf_dir'], f"{config['species']}_{config['assembly_version']}.filtered.woMD.vcf.gz.csi"),
		sites_file=os.path.join(config['paths']['reseq_filtered_vcf_dir'], f"{config['species']}_{config['assembly_version']}.filtered.missing_data.sites")
	output:
		os.path.join(config['paths']['reseq_filtered_vcf_dir'], f"{config['species']}_{config['assembly_version']}.filtered.vcf.gz")
	resources:
		mem_mb=8000
