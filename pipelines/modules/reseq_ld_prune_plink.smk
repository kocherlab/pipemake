...
rule reseq_indep_pairwise_plink:
    ...
    log:
        f"logs/plink/{config['species']}_{config['assembly_version']}.indep_pairwise.log",
    params:
        bed_prefix=subpath(input[0], strip_suffix=".bed"),
        out_prefix=subpath(output[0], strip_suffix=".prune.in"),
    ...
    threads: 1
    shell:
        "plink2 --bfile {params.bed_prefix} --indep-pairwise {params.ld_window_size} {params.ld_window_step} {params.ld_threshold} --bad-ld --out {params.out_prefix} --allow-extra-chr --threads {threads} &> {log}"
...
rule reseq_ld_prune_plink:
    ...
    output:
        ...
        f"reSEQ/PLINK/Pruned/{config['species']}_{config['assembly_version']}.pruned.fam",
    log:
        f"logs/plink/{config['species']}_{config['assembly_version']}.ld_prune.log",
    params:
        bed_prefix=subpath(input.bed, strip_suffix=".bed"),
        output_prefix=subpath(output[0], strip_suffix=".bed"),
    singularity:
        "docker://aewebb/plink2:20240418"
    resources:
        mem_mb=2000,
    threads: 1
    shell:
        "plink2 --bfile {params.bed_prefix} --extract {input.prune_in} --make-bed --out {params.output_prefix} --allow-extra-chr --threads {threads}"
