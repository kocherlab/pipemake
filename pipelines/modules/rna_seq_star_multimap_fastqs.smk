...
rule all:
    input:
        expand("RNAseq/BAM/Aligned/{sample}.Aligned.out.bam", sample=config["samples"]),
...
rule star_single_end:
    ...
    output:
        "RNAseq/BAM/Aligned/{sample}.Unmapped.out.mate1.gz",
...
rule star_pair_end:
    ...
    output:
        "RNAseq/BAM/Aligned/{sample}.Unmapped.out.mate1.gz",
        "RNAseq/BAM/Aligned/{sample}.Unmapped.out.mate2.gz",
        "RNAseq/BAM/Aligned/{sample}.Log.final.out",
    params:
        index_dir="Indices/STAR",
        bam_prefix="RNAseq/BAM/Aligned/{sample}.",
